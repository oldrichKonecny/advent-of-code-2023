...
fn compute_card_matches(line: &str) -> u32 {
    let (winnings, actual) = line.split_once(":").unwrap().1
        .split_once("|").unwrap();
    let winnings = winnings.trim().split_whitespace()
        .map(|n| n.parse::<u32>().unwrap())
        .collect::<HashSet<_>>();
    let actual = actual.trim().split_whitespace()
        .map(|n| n.parse::<u32>().unwrap())
        .collect::<Vec<_>>();

    actual.iter()
        .filter(|n| winnings.contains(n))
        .count() as u32
}
