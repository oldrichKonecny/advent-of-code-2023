...
impl Grid {
    ...
    fn check_and_compute_node_info_ultra(&mut self, y: i32, x: i32, direction: Direction, prev_node_info: &NodeInfo) -> Option<NodeInfo> {
        if y < 0 || x < 0 {
            return None;
        }
        let node = self.grid.get_mut(y as usize).and_then(|line| line.get_mut(x as usize));
    ...
    fn shortest_path_ultra(&mut self, og_y: i32, og_x: i32) {
        ...
        while let Some(node_info) = queue.pop_front() {
            ...
            if let Some(up_node_info) = self.check_and_compute_node_info_ultra(y - 1, x, Direction::Up, &node_info) {
                queue.push_back(up_node_info);
            }
            if let Some(down_node_info) = self.check_and_compute_node_info_ultra(y + 1, x, Direction::Down, &node_info) {
                queue.push_back(down_node_info);
            }
            if let Some(left_node_info) = self.check_and_compute_node_info_ultra(y, x - 1, Direction::Left, &node_info) {
                queue.push_back(left_node_info);
            }
            if let Some(right_node_info) = self.check_and_compute_node_info_ultra(y, x + 1, Direction::Right, &node_info) {
                queue.push_back(right_node_info);
            }
        }
    }
